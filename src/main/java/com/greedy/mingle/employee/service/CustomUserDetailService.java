...
@Service
@Slf4j 
@Transactional(readOnly = true) // 메서드에 트랜잭션 설정 추가
public class CustomUserDetailService implements UserDetailsService{
	...
	private EmployeeDTO addAuthorities(Employee employee) {
		...
		employeeDTO.setAuthorities(employee.getEmpRole().stream()
				.map(role -> new SimpleGrantedAuthority(role.getAuth().getAuthName()))
				.collect(Collectors.toList()));
		
		// empRole 설정
		employeeDTO.setEmpRole(employee.getEmpRole().stream()
